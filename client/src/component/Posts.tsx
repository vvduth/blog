...
const Posts: FC<any> = (props) => {
  ...
  const [stateLocal, setState] = useState({
    ...
    max_page: null as any,
    items_per_page: 3,

    currentPage: 1,
    num_posts: null,
    posts_slice: null as any,
    posts_search: [],
    posts_per_page: 3,
  });
  ...
  const add_search_post_to_state = (posts:any) => {
    setState({...stateLocal, posts_search: [] as any})
    setState({...stateLocal, posts_search: [...posts] as any})
  }

  const handleSearch = async (e: { preventDefault: () => void; target: any }) => {
    setState({...stateLocal, posts_search:[]}) ;
    const search_query = e.target.value
    await axios.get('/api/get/searchpost', {params: {search_query: search_query} })
       .then(res => res.data.length !== 0
                      ? add_search_post_to_state(res.data)
                      : null )
            ...
            <div className="FlexColumn">
              ...
              <div className="FlexRow">
                ...
                <Link style={{paddingLeft: '5px', textDecoration: 'none'}}
                      to={{pathname:"/user/" + post.post.author}}>
                 { post.post.author }
                 </Link>
               </div>
               <div className="FlexRow">
      ...
      <hr />
       ...
       <div>
         {stateLocal.posts_search
           ? stateLocal.posts_search.map((post:any) =>
      ...
      <h1>Posts</h1>
        <div>
          {stateLocal.posts_slice
            ? stateLocal.posts_slice.map((post:any) =>
